...
def _log_classification_stats(pred_logits, gt_classes, prefix="fast_rcnn"):
    ...
    tp = (fg_gt_classes == fg_pred_classes).sum().item()
    tn = ((gt_classes < 0) & (pred_classes == bg_class_ind)).sum().item()
    ...
    storage.put_scalar(f"{prefix}/precision", precision)
    storage.put_scalar(f"{prefix}/recall", recall)
    storage.put_scalar(f"{prefix}/f1_score", f1_score)
    if num_fg > 0:
